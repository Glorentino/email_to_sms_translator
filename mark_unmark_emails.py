...
# mark_as_read(service, "from: email@domain.com")

def mark_as_unread(service, query):
    message_to_mark = search_messages(service, query)
    print(f"Matched emails: {len(message_to_mark)}")
    return service.users().messages().batchModify(
        userId='me',
        body={
            'ids': [ msg['id'] for msg in message_to_mark],
            'addLabelIds': ['UNREAD']
        }
    ).execute()
